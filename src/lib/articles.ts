...
export const SAMPLE = {
  ...
  line: "It shows what the article template does. It is not published and not indexed.",
  art: "a desk with a notebook and a phone, mid-work",
  body: [
    {
      kind: "p" as const,
      text: "This page exists so the article template can be looked at before anything real is written. It is not published, it is not in the sitemap, and search engines are told not to index it.",
    },
    {
      kind: "h2" as const,
      text: "What the template gives you",
    },
    {
      kind: "p" as const,
      text: "A reading column about 65 characters wide, which is the width most people read fastest at. Headings, body copy, lists and quotes, at sizes that hold together on a phone as well as a laptop.",
    },
    {
      kind: "quote" as const,
      text: "A pull quote sits like this. It takes the heading face, so it reads as emphasis rather than as somebody speaking.",
    },
    {
      kind: "h2" as const,
      text: "Why it says SAMPLE so loudly",
    },
    {
      kind: "p" as const,
      text: "Because a convincing sample is worse than an obvious one. Filler that looks like an article gets linked, quoted and eventually believed, and by then nobody remembers it was filler. The brief's first rule is that nothing on this site claims more than is true, and that applies to the writing as much as to the features.",
    },
    {
      kind: "h2" as const,
      text: "What goes here instead",
    },
    {
      kind: "p" as const,
      text: "Real guides, written by people who have run this kind of business. The titles planned so far are listed on the guides page, each marked coming soon.",
    },
  ],
};
